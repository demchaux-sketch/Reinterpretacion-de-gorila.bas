...
const double ESCALA_VELOCIDAD = 4.15;
const double DT_FISICA = 0.016;
const int SUBPASOS_FISICA = 4;
...
void actualizarEtiquetasDisparo() {
    escribirTexto(etiquetaAngulo, ("Angulo: " + to_string(juego.anguloActual)).c_str());
    escribirTexto(etiquetaFuerza, ("Fuerza: " + to_string(juego.fuerzaActual)).c_str());
}
...
void nuevaRonda() {
    ...
    juego.anguloActual = 45;
    juego.fuerzaActual = 60;
    ...
    juego.mensaje = "Turno de " + juego.jugadores[0].nombre + ". Ajuste con flechas o WASD y dispare con Espacio/Enter.";

    limpiarEntradasDisparo();
    actualizarEtiquetasDisparo();
...
void calcularOrigenProyectil(int atacante, double& x, double& y) {
    const Jugador& jugador = juego.jugadores[atacante];
    int direccion = (atacante == 0) ? 1 : -1;
    int frameFinal = 7;

    x = jugador.x + direccion * (28 + frameFinal * 5);
    y = jugador.y - 38 - frameFinal * 4;
}

void calcularVelocidadInicial(int atacante, int angulo, int fuerza, double& vx, double& vy) {
    int direccion = (atacante == 0) ? 1 : -1;
    double radianes = angulo * PI / 180.0;

    vx = direccion * fuerza * ESCALA_VELOCIDAD * cos(radianes);
    vy = -fuerza * ESCALA_VELOCIDAD * sin(radianes);
}

bool segmentoTocaEdificio(double x1, double y1, double x2, double y2) {
    for (int i = 0; i <= 16; i++) {
        double t = i / 16.0;
        int x = static_cast<int>(round(x1 + (x2 - x1) * t));
        int y = static_cast<int>(round(y1 + (y2 - y1) * t));

        if (puntoTieneEdificio(x, y)) {
            return true;
        }
    }
...
void pasarTurnoDespuesDeFallo(HWND ventana) {
    ...
    juego.mensaje = "Turno de " + juego.jugadores[juego.turno].nombre + ". Ajuste con flechas o WASD.";
    actualizarEtiquetasDisparo();
...
void lanzarProyectil(HWND ventana, int angulo, int velocidad) {
    ...
    calcularOrigenProyectil(juego.turno, juego.proyectil.x, juego.proyectil.y);
    juego.proyectil.anteriorX = juego.proyectil.x;
    juego.proyectil.anteriorY = juego.proyectil.y;
    calcularVelocidadInicial(juego.turno, angulo, velocidad, juego.proyectil.vx, juego.proyectil.vy);
...
void iniciarDisparoConValores(HWND ventana, int angulo, int velocidad) {
    ...
    juego.anguloActual = angulo;
    juego.fuerzaActual = velocidad;
    actualizarEtiquetasDisparo();
...
void iniciarDisparo(HWND ventana) {
    ...
    iniciarDisparoConValores(ventana, juego.anguloActual, juego.fuerzaActual);
}
...
double distanciaAlObjetivo(int atacante, int angulo, int velocidad) {
    ...
    calcularOrigenProyectil(atacante, x, y);
    calcularVelocidadInicial(atacante, angulo, velocidad, vx, vy);

    for (int paso = 0; paso < 520; paso++) {
        double anteriorX = x;
        double anteriorY = y;
        vx += juego.viento * DT_FISICA;
        vy += GRAVEDAD * DT_FISICA;
        x += vx * DT_FISICA;
        y += vy * DT_FISICA;
        ...
        if (segmentoTocaEdificio(anteriorX, anteriorY, x, y) || x < 0 || x > VENTANA_ANCHO || y > SUELO_Y) {
            break;
        }
    }
...
void actualizarFisica(HWND ventana) {
    ...
    for (int i = 0; i < SUBPASOS_FISICA; i++) {
        double dt = DT_FISICA / SUBPASOS_FISICA;
        juego.proyectil.anteriorX = juego.proyectil.x;
        juego.proyectil.anteriorY = juego.proyectil.y;
...
void dibujarTrayectoriaPrevista(HDC hdc) {
    if (juego.enMenu || juego.pantallaFinal || juego.rondaTerminada ||
        juego.proyectil.activo || juego.lanzamiento.activo || juego.explosion.activa ||
        (juego.contraComputadora && juego.turno == 1)) {
        return;
    }
    ...
    double vx;
    double vy;
    calcularOrigenProyectil(juego.turno, x, y);
    calcularVelocidadInicial(juego.turno, juego.anguloActual, juego.fuerzaActual, vx, vy);
    ...
    for (int paso = 0; paso < 150; paso++) {
        ...
        if (paso % 7 == 0) {
            COLORREF color = (paso < 70) ? RGB(255, 236, 94) : RGB(238, 148, 70);
            int px = static_cast<int>(round(x));
            int py = static_cast<int>(round(y));
            dibujarRectPixel(hdc, px - 2, py - 2, 4, 4, color);
        }

        if (segmentoTocaEdificio(anteriorX, anteriorY, x, y) ||
            x < 0 || x > VENTANA_ANCHO || y > SUELO_Y) {
            break;
        }
    }
}
...
void dibujarMenu(HDC hdc) {
    ...
    dibujarTexto(hdc, 250, 310, "Flechas/WASD ajustan el tiro. Espacio/Enter dispara.", RGB(210, 226, 216));
...
void dibujarReglas(HDC hdc) {
    ...
    dibujarTexto(hdc, 245, 250, "2. Izq/Der o A/D cambian el angulo.", RGB(210, 226, 216));
    dibujarTexto(hdc, 245, 280, "3. Arriba/Abajo o W/S cambian la fuerza.", RGB(210, 226, 216));
    dibujarTexto(hdc, 245, 310, "4. Espacio o Enter dispara la banana.", RGB(210, 226, 216));
    dibujarTexto(hdc, 245, 340, "5. La linea punteada muestra la trayectoria.", RGB(210, 226, 216));
    dibujarTexto(hdc, 245, 370, "6. El viento modifica el vuelo y los edificios se rompen.", RGB(210, 226, 216));
    dibujarTexto(hdc, 245, 400, "7. Gana la partida quien llega primero a 3 puntos.", RGB(210, 226, 216));
...
void dibujarJuego(HDC hdc) {
    ...
    dibujarFondoPixelArt(hdc);
    dibujarCiudad(hdc);
    dibujarTrayectoriaPrevista(hdc);
...
bool jugadorPuedeControlarDisparo() {
    return !juego.enMenu && !juego.pantallaFinal && !juego.rondaTerminada &&
           !juego.proyectil.activo && !juego.lanzamiento.activo &&
           !(juego.contraComputadora && juego.turno == 1);
}

bool ajustarDisparoConTeclado(HWND ventana, WPARAM tecla) {
    if (!jugadorPuedeControlarDisparo()) {
        return false;
    }

    bool cambio = false;
    int pasoAngulo = (GetKeyState(VK_SHIFT) & 0x8000) ? 5 : 1;
    int pasoFuerza = (GetKeyState(VK_SHIFT) & 0x8000) ? 10 : 2;

    if (tecla == VK_LEFT || tecla == 'A') {
        juego.anguloActual = max(0, juego.anguloActual - pasoAngulo);
        cambio = true;
    } else if (tecla == VK_RIGHT || tecla == 'D') {
        juego.anguloActual = min(90, juego.anguloActual + pasoAngulo);
        cambio = true;
    } else if (tecla == VK_UP || tecla == 'W') {
        juego.fuerzaActual = min(120, juego.fuerzaActual + pasoFuerza);
        cambio = true;
    } else if (tecla == VK_DOWN || tecla == 'S') {
        juego.fuerzaActual = max(10, juego.fuerzaActual - pasoFuerza);
        cambio = true;
    } else if (tecla == VK_SPACE || tecla == VK_RETURN) {
...
LRESULT CALLBACK procedimientoVentana(HWND ventana, UINT mensaje, WPARAM wParam, LPARAM lParam) {
    switch (mensaje) {
        case WM_CREATE:
            etiquetaAngulo = CreateWindowA("STATIC", "Angulo: 45", WS_VISIBLE | WS_CHILD, 545, 15, 105, 22,
            ...
            etiquetaFuerza = CreateWindowA("STATIC", "Fuerza: 60", WS_VISIBLE | WS_CHILD, 660, 15, 125, 22,
                                           ventana, NULL, NULL, NULL);
        ...
        case WM_KEYDOWN:
            ajustarDisparoConTeclado(ventana, wParam);
            return 0;
...
int WINAPI WinMain(HINSTANCE instancia, HINSTANCE, LPSTR, int mostrar) {
    ...
    while (GetMessageA(&mensaje, NULL, 0, 0)) {
        if (mensaje.message == WM_KEYDOWN && ajustarDisparoConTeclado(ventana, mensaje.wParam)) {
            continue;
        }
